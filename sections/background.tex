...
The iCub humanoid robot features a total of 53 degrees of freedom (DOF) distributed throughout its body. 
Specifically, each leg possesses 6 DOFs, the torso contains 3 DOFs, the head and eyes system incorporates 6 DOFs, each arm utilizes 7 DOFs, and each hand employs 9 DOFs. Additionally, a dedicated servo motor controls the opening and closing of the robot's eyelids.
This sophisticated mechanical design places the iCub among the most advanced humanoid platforms in robotics research today.
Developed by the Italian Institute of Technology (IIT) through the European Commission's RobotCub project, the iCub was conceived with a specific vision: to create an open-source humanoid platform that could serve as a common tool for researchers investigating embodied cognition, developmental robotics, and human-robot interaction. Standing approximately 104 centimeters tall, the iCub's size and proportions intentionally mimic those of a 3-4 year old child, reflecting its conceptual roots in developmental psychology and cognitive science.
The name iCub itself reveals this inspiration, with "Cub" referencing its childlike form and capabilities, while the preceding "i" acknowledges both its Italian origins and its place in modern technological innovation. This etymological foundation underscores the robot's purpose as a platform for exploring how intelligence emerges through physical interaction with the environment—a cornerstone principle of embodied cognition.
Beyond its mechanical complexity, the iCub incorporates an impressive array of sensory systems. Stereo vision is enabled through cameras positioned in the robot's eyes, providing depth perception and visual processing capabilities. Tactile sensing is distributed across the body, with particularly dense sensor arrays in the fingertips and palms, allowing for sophisticated manipulation and object recognition. Additional proprioceptive sensors monitor joint positions and forces, while auditory inputs enable sound localization and voice recognition.
The iCub's cognitive architecture is built upon YARP (Yet Another Robot Platform), an open-source middleware that facilitates modular software development and hardware abstraction. This foundation supports a growing ecosystem of software components addressing everything from low-level motor control to high-level cognitive functions like attention, memory, and learning.
What distinguishes the iCub from many other humanoid platforms is its developmental approach to artificial intelligence. Rather than programming fixed behaviors, researchers using the iCub often implement learning algorithms that allow the robot to acquire skills through exploration and interaction—mirroring the developmental trajectory of human children. This approach has led to demonstrations of capabilities ranging from object manipulation and tool use to rudimentary forms of social interaction and language acquisition.
The open-source nature of both the iCub's hardware designs and software has fostered a vibrant international research community. Since its initial release in 2008, more than 40 iCub units have been deployed to laboratories across Europe, Asia, and North America. This distribution has enabled collaborative research efforts addressing fundamental questions in cognitive science, neuroscience, and artificial intelligence.
As robotics and AI continue to advance, the iCub remains at the forefront of research investigating how embodied, developmental approaches might lead to more robust and adaptable intelligent systems. Its comprehensive design, combining sophisticated mechanics with advanced sensing and cognitive capabilities, provides a unique platform for exploring the complex interplay between physical form, sensorimotor experience, and the emergence of intelligence.
...
\begin{equation}\label{eq:contact_wrench_projection}
...
dynamic walker can be described by a set of ordinary differential equations that govern the motion of the robot. 

\section{Mathematical foundations}
The dynamics of a passive walker can be described through a mixed dynamical system approach, combining continuous swing-phase dynamics of the legs with discrete impact events when the foot enters in contact with the ground.
For a simple compass gait model, for example, which consists of a point mass body and two point mass legs, this motion can be described by a set of coupled differential equations derived from the Lagrangian mechanics:

\begin{equation}
    M(q)\ddot{\vec{q}} + C(\vec{q}, \dot{\vec{q}})\dot{\vec{q}} + G(\vec{q}) = 0,
\end{equation}
where $M(\theta)$ represents the inertia matrix, $C(\theta, \dot{\theta})$ accounts for Coriolis and centrifugal effects, and $G(\theta)$ describes gravitational forces.
The vector $\vec{q}$ for such model typically includes the angle of the stance leg and the relative inter-leg angle.
